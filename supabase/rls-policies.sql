...
-- Clean Public Schema Helpers (No auth schema permission errors)
-- ============================================================================

-- 1. Helper function to extract current authenticated user's role from profiles table
...
CREATE POLICY "leaves_insert" ON student_leaves
    FOR INSERT WITH CHECK (
        public.is_admin()
        OR public.current_user_role() IN ('FACULTY', 'STUDENT')
    );
...
CREATE POLICY "leaves_update" ON student_leaves
    FOR UPDATE USING (
        public.is_admin()
        OR public.current_user_role() IN ('FACULTY', 'WARDEN')
    );
...
CREATE POLICY "exams_select" ON online_exams
    FOR SELECT USING (
        is_published = true OR public.is_admin() OR public.current_user_role() = 'FACULTY'
    );

DROP POLICY IF EXISTS "exams_faculty_manage" ON online_exams;
CREATE POLICY "exams_faculty_manage" ON online_exams
    FOR ALL USING (public.is_admin() OR public.current_user_role() = 'FACULTY');
